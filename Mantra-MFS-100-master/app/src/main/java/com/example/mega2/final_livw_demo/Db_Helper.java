...
class Db_Helper extends SQLiteOpenHelper {
    ...
    private static final String SQL_CREATE_TABLE_1 = "CREATE TABLE " + USER_MASTER + " (" + USER_MASTER_id + " INTEGER PRIMARY KEY AUTOINCREMENT, " + USER_MASTER_USERID + " VARCHAR(20)," + USER_MASTER_NAME + " VARCHAR(50)," + USER_MASTER_VERIFYMODE + " VARCHAR(2)," + USER_MASTER_USERTYPE + " VARCHAR(2) , " + USER_MASTER_EXPIRY + " DATE , " + USER_MASTER_DOB + " VARCHAR(30) , " + USER_MASTER_PASSWORD + " VARCHAR(20) , " + USER_MASTER_IS_SYNC + " VARCHAR(1) , " + USER_MASTER_IMAGE + " VARCHAR(20) )";
    ...
    boolean insert_USER_Data(String UserId, String Name, String Verifymode, String Usertype, String date, String pass ,String Image) {
        ...
        contentValues.put(USER_MASTER_PASSWORD, pass);
        contentValues.put(USER_MASTER_IMAGE,Image);
        long result = db.insert(USER_MASTER, null, contentValues);
        return result != -1;
    }
    boolean insert_USER_Data_from_csv(String UserId, String Name, String Verifymode, String Usertype, String date) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put(USER_MASTER_USERID, UserId);
        contentValues.put(USER_MASTER_NAME, Name);
    ...
    String getallData(String s) {
        SQLiteDatabase db = this.getWritableDatabase();
        String[] columns = {USER_MASTER_id, USER_MASTER_NAME, USER_MASTER_USERID, USER_MASTER_VERIFYMODE,USER_MASTER_USERTYPE,USER_MASTER_DOB};
        ...
        while (cursor.moveToNext()) {
            ...
            String verifymode = cursor.getString(3);
            ...
            pojo.setVerifyMode(String.valueOf(verifymode.charAt(0)));
            pojo.setUserType(String.valueOf(usertype.charAt(0)));
            arrayList.add(pojo);
            buffer.append(userid + "/" + name + "/" + verifymode + "/" + usertype + "/" + dob + "\n");
        }
        cursor.close();
        return buffer.toString();
    }

    long modifyDetails(String _id, String UserId, String Name, String Usertype, String Verifymode, String Date_of_birth, String pass,String filename)
    {
        SQLiteDatabase db=this.getWritableDatabase();
        ContentValues contentValues=new ContentValues();
        contentValues.put(USER_MASTER_USERID,UserId);
        contentValues.put(USER_MASTER_NAME,Name);
        contentValues.put(USER_MASTER_USERTYPE,Verifymode);
        contentValues.put(USER_MASTER_VERIFYMODE,Usertype);
    ...
    ArrayList<MyPojo> getAllData(String UserID) {
        ...
        SQLiteDatabase db = this.getWritableDatabase();
        if (UserID.equals("")) {
           querry = "select um._id,um.userid,name,verifymode,usertype,date_of_birth,count(ud.userid) as FCnt,Password,Image from user_master um left outer join user_detail ud on um.userid=ud.userid group by um._id,um.userid,name,verifymode,usertype,date_of_birth order by um.UserId asc";
        }
        else
        {
            querry = "select um._id,um.userid,name,verifymode,usertype,date_of_birth,count(ud.userid) as FCnt,Password,Image from user_master um left outer join user_detail ud on um.userid=ud.userid Where um.UserId='" + UserID +"' group by um._id,um.userid,name,verifymode,usertype,date_of_birth order by um.UserId asc";
        }
    ...
    {
        SQLiteDatabase db=this.getWritableDatabase();
        String[] columns = {USER_MASTER_USERID, USER_MASTER_PASSWORD};
        Cursor cursor = db.query(USER_MASTER, columns, USER_MASTER_USERID + "=?", new String[]{userid}, null, null, null);
        StringBuilder buffer = new StringBuilder();
        while (cursor.moveToNext()) {
    ...
    String getVData(String s) {
        ...
        Cursor cursor=db.rawQuery("select usertype from USER_DETAIL ud inner join USER_MASTER um on ud.userid=um.userid and ud.file = '" + s + "'",null);
        while (cursor.moveToNext())
        {
